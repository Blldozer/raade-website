...
const ProjectsShowcase = () => {
  ...
  const filteredProjects = projects.filter(project => selectedSector === "All" || project.sector === selectedSector || project.sectors?.includes(selectedSector));
        ...
        <div className="flex flex-col lg:flex-row mb-20">
          ...
          duration: 0.6
            ...
            <div className="space-y-8 text-xl leading-relaxed font-lora text-gray-700 max-w-[800px]">
              <p className="text-gray-700">We focus on high-impact sectors where innovative solutions can create transformative change at scale. Each project addresses challenges affecting millions of people across Africa, designed not just to solve immediate problems, but to create ripple effects that transform entire communities and markets. Here are our 2024-2025 challenges.</p>
            </div>
          </motion.div>
        </div>

        {/* Filter Buttons */}
        <ProjectFilter sectors={sectors} selectedSector={selectedSector} onSectorChange={handleSectorChange} />

        {/* Projects Grid */}
        <motion.div variants={containerVariants} initial="hidden" whileInView="visible" viewport={{
        once: true
      }} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-10" key={selectedSector} // Add key to force re-render when sector changes
      >
          {filteredProjects.map(project => <ProjectCard key={project.name} project={project} setHoveredProject={setHoveredProject} itemVariants={itemVariants} />)}
        </motion.div>
      </div>
    </section>;
};
