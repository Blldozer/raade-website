    ...
    <div>
      ...
      >
        ...
        <SelectContent>
          ...
          <SelectItem value={TICKET_TYPES_ENUM.SPECIAL_BONUS}>Special Bonus {getTicketPriceText(TICKET_TYPES_ENUM.SPECIAL_BONUS)}</SelectItem>
        </SelectContent>
      </Select>
      {errors.ticketType && (
      ...
      {watchTicketType === TICKET_TYPES_ENUM.SPECIAL_BONUS && (
        <p className="text-gray-600 text-sm mt-1">
          Special bonus tickets available for a limited time.
        </p>
      )}
    </div>
  );
