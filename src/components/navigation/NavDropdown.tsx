...
const NavDropdown = ({ name, href, dropdownItems, textColor = "text-white", onClick }: NavDropdownProps) => {
  ...
  if (typeof React !== 'object') {
    console.warn("NavDropdown: React object unavailable");
    return null;
  }

  try {
    // Use navigation hook for routing
    ...
    const handleMainClick = (e: React.MouseEvent) => {
      ...
      try {
        ...
        console.error("NavDropdown: Navigation error", error);
      }
    };
    ...
    console.warn("NavDropdown: React context unavailable, providing minimal fallback", error);
    ...
    return (
      <div className="relative">
        <a 
          href={href}
          className={`${textColor} flex items-center text-lg font-bold`}
        >
          {name}
        </a>
      </div>
