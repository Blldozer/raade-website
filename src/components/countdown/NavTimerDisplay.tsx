...
const NavTimerDisplay = ({ 
  ...
  if (!isReactAvailable) {
    console.error("NavTimerDisplay: React hooks unavailable, rendering fallback");
    // Return a minimal fallback that won't crash
    return (
      <div className={cn("font-alegreyasans font-bold", colors.text, className)}>
        Conference: Apr 11-12
      </div>
    );
  }
  
  try {
    // Safe use of React hooks
    ...
    console.error("NavTimerDisplay: React context error", error);
    // Return a minimal fallback that won't crash
    return (
      <div className={cn("font-alegreyasans font-bold", colors.text, className)}>
        Conference: Apr 11-12
